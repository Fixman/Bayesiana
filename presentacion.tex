...
\[
W_{PT/CT} = \log \left( P \left( PT \mid X \right) / P \left( CT \mid X \right) \right)
\]

\alt<2>{%
Cuando $W_{PT/CT}$ es fuertemente negativo, la contrubución de \( PT \) es insignificante, y el agente se comporta como si fuese un \textbf{Copy Theorist}.

Si la evidencia se acumula y cambia a $W_{PT/CT}$ para que sea fuertemente positivo, el agente se comporta como un \textbf{Perspective Theorist}.
}{}
...
\caption{\( W_{PT/CT} \), evaluada en juicios\footnotemark{} de experiencia. Cada parte consiste en ensayos con \( \left( W, V, A \right) \) observables, pero \( \left( D, B \right) \) ocultos.}

\footnotetext{``\textit{Epoch}'' en el paper, y no encontramos una traducción perfecta}
\end{figure}
